        ...
        <section className="qh-container qh-section-pad">
          ...
          <div className="mt-8">
            <Link
              ...
              className="group block overflow-hidden rounded-2xl border border-[#E6E7E8] bg-white shadow-soft transition-all duration-300 hover:-translate-y-1 hover:shadow-md"
            >
              <div className="grid items-stretch divide-y divide-[#E6E7E8] md:grid-cols-12 md:divide-x md:divide-y-0">
                {/* Info and Banner block */}
                <div className="flex flex-col justify-between bg-gradient-to-br from-[#F7F5FD] via-white to-transparent p-6 md:col-span-5">
                  <div>
                    <span className="rounded-full bg-[#432F83]/10 px-3 py-1 text-xs font-bold text-[#432F83]">Collection Spotlight</span>
                    <h3 className="mt-4 font-display text-2xl font-black text-[#231F20] transition-colors group-hover:text-[#432F83]">{col.name}</h3>
                  ...
                  <div className="mt-6 flex items-center justify-between border-t border-[#E6E7E8] pt-4">
                    <span className="text-xs font-semibold text-text-soft">{col.products.length} product(s) inside</span>
                    <span className="text-sm font-bold text-[#432F83] transition-transform group-hover:translate-x-1">View Collection →</span>
                  </div>
                </div>
                {/* Products grid preview */}
                <div className="flex items-center justify-center bg-[#F7F5FD]/50 p-6 md:col-span-7">
                  {col.products && col.products.length > 0 ? (
                    <div className="grid grid-cols-3 gap-3 w-full">
                      {col.products.slice(0, 3).map((p, i) => (
                        <div key={p.slug} className="relative aspect-square overflow-hidden rounded-xl border border-[#E6E7E8] bg-white shadow-xs">
                          {p.image_url ? (
                            <img
                              src={p.image_url}
                              alt={p.title}
                            ...
                            <div className="absolute inset-0 flex items-center justify-center bg-[#F7F5FD] text-[10px] text-[#575757]">
                              No image
                            </div>
                          )}
                      ...
                      {col.products.length < 3 && Array.from({ length: 3 - col.products.length }).map((_, i) => (
                        <div key={i} className="aspect-square rounded-xl border border-dashed border-[#E6E7E8] bg-white/60" />
                      ))}
                    </div>
                  ) : (
      ...
      <div className="mt-8 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
        {collections.map((col) => (
          <Link
            key={col.id}
            href={`/collections/${col.slug}`}
            className="group overflow-hidden rounded-2xl border border-[#E6E7E8] bg-white shadow-soft transition-all duration-300 hover:-translate-y-1 hover:shadow-md"
          >
            {/* Image Grid */}
            <div className="grid h-48 grid-cols-3 gap-px bg-[#E6E7E8]">
              {col.products.slice(0, 3).map((p, i) => (
                <div key={p.slug} className="relative overflow-hidden bg-[#F9FAFC]">
                  {p.image_url ? (
                    <img
                      src={p.image_url}
                      alt={p.title}
                    ...
                    <div className="flex h-full items-center justify-center text-xs text-[#575757]">No image</div>
                  )}
                </div>
              ))}
              {col.products.length < 3 &&
                Array.from({ length: 3 - col.products.length }).map((_, i) => (
                  <div key={`empty-${i}`} className="bg-[#F9FAFC]" />
                ))}
            </div>
            {/* Info */}
            <div className="p-4">
              <h3 className="text-lg font-black text-[#231F20] transition-colors duration-fast group-hover:text-[#432F83]">
                {col.name}
              </h3>
              {col.description && (
                <p className="mt-1 text-sm text-text-muted line-clamp-2">{col.description}</p>
              )}
              <p className="mt-2 text-sm font-semibold text-[#432F83]">
